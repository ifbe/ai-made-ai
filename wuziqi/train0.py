...
def find_latest_checkpoint():
    """查找最新的检查点"""
    checkpoint_files = glob.glob("wuziqi_stage0_checkpoint_epoch*.pth")
...
class Stage0Trainer:
    ...
    def train_step(self, batch):
        ...
        for item in batch:
            ...
            fear_labels.append(item.get('fear_label', None))
            greed_labels.append(item.get('greed_label', None))
        ...
        # 前向传播，获取详细信息
        details = self.model.forward_with_mask(boards, turns, return_details=True)
        ...
        # 辅助损失1：恐惧分数（如果有标签）
        if any(f is not None for f in fear_labels):
            fear_targets = []
            for i, item in enumerate(batch):
                if fear_labels[i] is not None:
                    fear_targets.append(torch.tensor(fear_labels[i], device=self.device))
                else:
                    # 如果没有恐惧标签，用全0
            ...
            total_loss += 0.2 * fear_loss  # 权重0.2
        
        # 辅助损失2：贪婪分数（如果有标签）
        if any(g is not None for g in greed_labels):
            greed_targets = []
            for i, item in enumerate(batch):
                if greed_labels[i] is not None:
                    greed_targets.append(torch.tensor(greed_labels[i], device=self.device))
                else:
            ...
            total_loss += 0.2 * greed_loss  # 权重0.2
    ...
    def evaluate_accuracy(self, dataset, num_samples=200):
        ...
        with torch.no_grad():
            for item in samples:
                ...
                turn_tensor = torch.tensor([turn], device=self.device)
...
def main():
    ...
    for epoch in range(trainer.start_epoch, args.epochs + 1):
        ...
        if epoch % 5 == 0:
            ...
            if val_acc > trainer.best_acc:
                ...
                torch.save(model.cpu().state_dict(), f"wuziqi_stage0_best_{val_acc:.0%}_epoch{epoch}.pth")
                model.to(device)
    ...
    torch.save(model.cpu().state_dict(), "wuziqi_stage0_final.pth")
